...
def default_us_pipeline_stage_contracts() -> tuple[USPipelineStageContract, ...]:
    ...
    return (
        ...
        USPipelineStageContract(
            id="04_seed_and_donors",
            step="04",
            title="Seed construction and donor integration",
            purpose="Project the scaffold into the seed schema and integrate donor variables.",
            consumes=("source plan", "scaffold frame", "donor frames"),
            produces=("seed frame", "donor integration metadata"),
            artifacts=(
                USStageArtifactContract(
                    key="seed_data",
                    description="Seed population after scaffold projection and donor integration.",
            ...
            resume_notes="The seed frame is saved, but downstream replay currently starts from the synthetic/candidate frame.",
        ),
        USPipelineStageContract(
            id="05_synthesis",
            step="05",
            title="Synthesis, candidate population, and support enforcement",
            purpose="Produce the candidate population that will be calibrated.",
            consumes=(
                "seed frame",
                "synthesis variable plan",
                "target support requirements",
            ),
            produces=("synthetic/candidate frame", "synthesis metadata"),
            artifacts=(
                USStageArtifactContract(
                    key="synthetic_data",
