...
_register(
    VariableMapping(
        ...
        entity="Person",
        notes="CPS A_AGE is as of survey date (March); A_AGE==80 represents ages 80-84.",
    )
)


def map_age(persons: pl.DataFrame) -> pl.DataFrame:
    """Map CPS A_AGE to age, spreading the 80-84 topcode."""
    return randomize_cps_topcoded_age_80_84(
        persons.with_columns(pl.col("A_AGE").alias("age"))
    )
...
def map_household_size(households: pl.DataFrame) -> pl.DataFrame:
    ...
    return households.with_columns(pl.col("H_NUMPER").alias("household_size"))
...
def map_earned_income(persons: pl.DataFrame) -> pl.DataFrame:
    ...
    return persons.with_columns(
        (pl.col("WSAL_VAL").fill_null(0) + pl.col("SEMP_VAL").fill_null(0)).alias(
            "earned_income"
        )
    )
...
def map_is_blind(persons: pl.DataFrame) -> pl.DataFrame:
    ...
    return persons.with_columns((pl.col("PEDISEYE") == 1).alias("is_blind"))
...
def map_is_dependent(persons: pl.DataFrame) -> pl.DataFrame:
    ...
    return persons.with_columns(
        ((pl.col("A_EXPRRP").is_in([4, 8])) & (pl.col("A_AGE") < 19)).alias(
            "is_dependent"
        )
    )
